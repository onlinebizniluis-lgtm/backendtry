...
window.renderLogbook = function() {
    // Check if variables exist to prevent crash if data isn't loaded yet
    if (typeof allTrades === 'undefined') return; 

    const fPair = document.getElementById("filter-pair")
      ? document.getElementById("filter-pair").value.toUpperCase()
      : "";
    const fRes = document.getElementById("filter-res") ? document.getElementById("filter-res").value : "ALL";
    const now = new Date();
    ...
    let filtered = allTrades.filter((t) => {
      ...
      let timeMatch = true;
      if (typeof logViewRange !== 'undefined' && logViewRange === "30")
    ...
    // Define View Mode Default if missing
    if (typeof logViewMode === 'undefined') window.logViewMode = "list";

    if (logViewMode === "list") {
      if(document.getElementById("log-list-view")) document.getElementById("log-list-view").style.display = "table";
      if(document.getElementById("log-calendar-view")) document.getElementById("log-calendar-view").style.display = "none";
      ...
      // CALENDAR VIEW LOGIC
      if(document.getElementById("log-list-view")) document.getElementById("log-list-view").style.display = "none";
      if(document.getElementById("log-calendar-view")) document.getElementById("log-calendar-view").style.display = "block";
      
      if (typeof logCalDate === 'undefined') window.logCalDate = new Date();
      ...
      if(document.getElementById("log-cal-month")) document.getElementById("log-cal-month").innerText = `${monthNames[month]} ${year}`;
      ...
      if(document.getElementById("log-cal-grid")) document.getElementById("log-cal-grid").innerHTML = html;
    }
  }

  window.viewTradeDetail = function(id) {
    ...
    if (document.getElementById("btn-open-chart")) {
      document.getElementById("btn-open-chart").onclick = () => {
        if (isValidUrl) {
          ...
          if(typeof sysNotify === "function") sysNotify("INVALID LINK FORMAT", "error");
        }
      };
    }
  }

  window.viewDayDetails = function(dateStr, trades) {
    const overlay = document.createElement("div");
    overlay.className = "sys-modal-overlay";
    let html = trades
      .map(
